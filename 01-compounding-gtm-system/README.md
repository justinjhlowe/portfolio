...
**The loop** is what makes the three spokes compound instead of running as three disconnected tools. `change-history.md` is the proof of this, not a description of it: five dated entries showing real reply rates, meeting outcomes, and win/loss reasons changing specific files with a stated reason each time. The single strongest moment in the eventual live demo is running the same signal-to-sequence workflow before and after one of these updates, same input, visibly different output. That's the difference between tool-centric and learning-centric, shown instead of claimed.
...
The repository and all three spokes are built and internally consistent — every number and claim in any demo traces to a specific file and, where relevant, a specific dated entry in `change-history.md`. What's not done: a single walkthrough that runs all three spokes back to back off one before/after toggle instead of three separate files, and `results.md` with real anonymized outcomes once this pattern has run for an actual client. Both are the right next step, not before-you-can-pitch blockers — the three demos as they stand are enough to run the full argument today.
